...
//!
//! Each app gets its own WSL2 distro (`craterun-<appId>`) with containerd
...
use craterun_core::types::ContainerState;
...
/// Namespace prefix for all CrateRun-managed WSL distros.
const DISTRO_PREFIX: &str = "craterun-";
...
impl WslContainerdAdapter {
    ...
    async fn wsl_exec(distro: &str, args: &[&str]) -> Result<std::process::Output, CrateRunError> {
        ...
        Command::new("wsl.exe")
            ...
            .output()
            .await
            .map_err(|e| CrateRunError::RuntimeError(format!("wsl exec failed: {}", e)))
    }
    ...
    async fn ensure_containerd(distro: &str) -> Result<(), CrateRunError> {
        ...
        if !start.status.success() {
            return Err(CrateRunError::RuntimeError(
...
impl RuntimeAdapter for WslContainerdAdapter {
    ...
    async fn prepare(&self) -> Result<(), CrateRunError> {
        ...
        // tarball at runtime-assets/craterun-base-distro.tar.gz
    ...
    async fn import_image(
        ...
        let output = Command::new("wsl.exe")
            ...
            .output()
            .await
            .map_err(|e| CrateRunError::ImageImportFailed {
        ...
        if !output.status.success() {
            ...
            return Err(CrateRunError::ImageImportFailed {
                service: image_name.to_string(),
    ...
    async fn start_app(&self, plan: &RuntimePlan) -> Result<(), CrateRunError> {
        ...
        for service in &plan.services {
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(CrateRunError::ContainerStartFailed {
    ...
    async fn create_network(&self, app_id: &AppId) -> Result<(), CrateRunError> {
        let distro = Self::distro_name(app_id);
        let network = format!("craterun-{}", app_id.as_str().replace('.', "-"));
        ...
        if !output.status.success() {
            ...
            if !stderr.contains("already exists") {
                return Err(CrateRunError::RuntimeError(format!(
    ...
    async fn remove_app(
        ...
        if options.remove_volumes {
            ...
            let prefix = format!("craterun-{}-", app_id.as_str().replace('.', "-"));
            let output = Self::wsl_exec(
                &distro,
                &[
                    "nerdctl",
        ...
        if options.remove_runtime {
            ...
            let output = Command::new("wsl.exe")
                ...
                .await
                .map_err(|e| {
                    CrateRunError::RuntimeError(format!("wsl unregister failed: {}", e))
                })?;
...
mod tests {
    ...
    fn distro_naming() {
        ...
        assert_eq!(
            ...
            "craterun-com-example-photobooth"
        );
    }
