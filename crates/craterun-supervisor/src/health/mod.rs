...
use craterun_core::CrateRunError;
...
impl HealthChecker {
    ...
    pub async fn wait_until_ready(&self) -> Result<(), CrateRunError> {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(5))
            .build()
            .map_err(|e| CrateRunError::RuntimeError(format!("http client error: {}", e)))?;

        let start = Instant::now();

        loop {
            if start.elapsed() > self.timeout {
                return Err(CrateRunError::HealthCheckTimeout {
