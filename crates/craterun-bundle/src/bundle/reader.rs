...
impl BundleReader {
    ...
    pub fn read_manifest(&self) -> Result<CompiledManifest, craterun_core::CrateRunError> {
    ...
    pub fn validate_structure(&self) -> Result<(), craterun_core::CrateRunError> {
        let required = ["manifest.json", "images"];
        for entry in required {
            let path = self.bundle_dir.join(entry);
            if !path.exists() {
                return Err(craterun_core::CrateRunError::BundleReadError(format!(
