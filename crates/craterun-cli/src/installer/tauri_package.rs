...
impl TauriPackager {
    ...
    pub fn stage_supervisor(&self) -> anyhow::Result<()> {
        ...
        #[cfg(windows)]
        let supervisor_name = "craterun-supervisor.exe";
        #[cfg(not(windows))]
        let supervisor_name = "craterun-supervisor";
...
fn find_supervisor_binary(workspace_root: &Path) -> anyhow::Result<PathBuf> {
    #[cfg(windows)]
    let names = ["craterun-supervisor.exe"];
    #[cfg(not(windows))]
    let names = ["craterun-supervisor"];
    ...
    anyhow::bail!(
        "supervisor binary not found in target/release or target/debug. \
         Run `cargo build --release -p craterun-supervisor` first."
    )
}
