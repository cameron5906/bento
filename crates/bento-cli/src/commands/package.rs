...
fn locate_binaries() -> anyhow::Result<InstallerBinaries> {
    let shell_names = ["bento-shell.exe", "bento_shell.exe"];
    let supervisor_names = ["bento-supervisor.exe", "bento_supervisor.exe"];

    let mut shell_exe = PathBuf::from("bento-shell.exe");
    let mut supervisor_exe = PathBuf::from("bento-supervisor.exe");
